 ...
 <tbody>              
  ...
  <c:forEach var="book" items="${sessionScope.books}">                      
   <tr align="center">                            
    ...
    <td><button onclick="window.location.href='AlterWant.jsp?bookname=${book.name}&writer=${book.author}&year=${book.publicYear}&ISBN=${book.pubNumber}&id=${book.id}'" id="alter" value="<c:out value="${book.id}" />">修改</button></td>
    <td><button onclick="deleteCurrentRow(this)" id="<c:out value="${book.id}" />" value="<c:out value="${book.id}" />">删除</button></td>                                                   
   </tr>              
  </c:forEach>
 </tbody>
</table>
